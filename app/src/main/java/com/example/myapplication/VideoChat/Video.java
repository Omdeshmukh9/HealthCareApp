...
public class Video extends AppCompatActivity {
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        // Initialize ListView for doctors' emails
        listViewDoctors = findViewById(R.id.listViewDoctors);

        // Retrieve doctors' emails from Firebase Firestore and populate the ListView
        FirebaseFirestore db = FirebaseFirestore.getInstance();
        db.collection("users_doctors")
                .get()
                .addOnCompleteListener(task -> {
                    if (task.isSuccessful()) {
                        List<String> doctorsEmails = new ArrayList<>();
                        for (DocumentSnapshot document : task.getResult()) {
                            String doctorEmail = document.getString("email");
                            if (doctorEmail != null) {
                                doctorsEmails.add(doctorEmail);
                            }
                        }
                        // Populate the list view with doctors' emails
                        ArrayAdapter<String> adapter = new ArrayAdapter<>(this,
                                android.R.layout.simple_list_item_1, doctorsEmails);
                        listViewDoctors.setAdapter(adapter);
                    } else {
                        Timber.d("Error getting documents: %s", task.getException());
                    }
                });

        // Set click listener for ListView items
        listViewDoctors.setOnItemClickListener((parent, view, position, id) -> {
            String selectedDoctorEmail = (String) parent.getItemAtPosition(position);
            TextInputLayout inputLayout = findViewById(R.id.target_user_id);
            inputLayout.getEditText().setText(selectedDoctorEmail);
        });
    }
